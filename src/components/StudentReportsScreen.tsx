...
import { ScrollText, FileText, Download, Printer, Phone, MessageCircle, Edit2, Check, X, Palette, Settings, Share2, Copy, Send, Sparkles, MessageSquare, Award } from 'lucide-react';
...
import { AnnualReportPdfModal } from './AnnualReportPdfModal';
...
export const StudentReportsScreen: React.FC<StudentReportsScreenProps> = ({ records, selectedTerm, teacher, onOpenStudentMessages }) => {
  ...
  // Annual Report PDF Modal State
  const [isAnnualReportOpen, setIsAnnualReportOpen] = useState(false);
  const [annualStudentId, setAnnualStudentId] = useState<string>('all');
    ...
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100 w-full animate-in fade-in printable-area">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-2 print:hidden">
          ...
          {/* Annual Cumulative PDF Report Button */}
          <button
            onClick={() => {
              setAnnualStudentId('all');
              setIsAnnualReportOpen(true);
            }}
            disabled={!selectedGrade || !selectedClassNum}
            className="px-3.5 py-2 bg-gradient-to-r from-indigo-600 via-purple-600 to-emerald-600 hover:from-indigo-700 hover:to-emerald-700 text-white rounded-lg text-xs font-black flex items-center gap-1.5 transition-all cursor-pointer shadow-sm active:scale-95 disabled:opacity-50"
            title="توليد تقرير أداء سنوي تراكمي لكل طالب (ترم أول + ترم ثاني PDF)"
          >
            <Award className="w-4 h-4 text-amber-300" />
            <span>التقرير السنوي التراكمي PDF</span>
          </button>
            ...
            {reportData.map((data) => (
              <div key={data.student.id} className="bg-white border border-slate-200 rounded-xl p-3 shadow-xs hover:shadow-md transition-shadow flex flex-col justify-between relative group">
                ...
                <div className="mt-3 space-y-1.5">
                  {/* Share Report & Annual Report Buttons */}
                  <div className="grid grid-cols-2 gap-1">
                    <button
                      type="button"
                      onClick={() => setSelectedShareData(data)}
                      className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold text-[10px] py-1.5 px-1 rounded-lg transition-all flex items-center justify-center gap-1 shadow-xs cursor-pointer active:scale-95"
                      title="مشاركة التقرير الشهري عبر الواتساب"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setAnnualStudentId(data.student.id);
                        setIsAnnualReportOpen(true);
                      }}
                      className="bg-slate-900 hover:bg-slate-800 text-white font-bold text-[10px] py-1.5 px-1 rounded-lg transition-all flex items-center justify-center gap-1 shadow-xs cursor-pointer active:scale-95"
                      title="توليد وتنزيل التقرير السنوي التراكمي (PDF)"
                    >
      ...
      <AnnualReportPdfModal
        ...
        initialStudentId={annualStudentId}
      />
